...
def _search(task, search, heuristic, max_depth:int, use_preferred_ops=False):
    logging.info(f"Search start: {task.name}")
    if heuristic:
        if use_preferred_ops:
            solution = search(task, heuristic, use_preferred_ops, max_depth)
        else:
            solution = search(task, heuristic, max_depth)
    else:
        solution = search(task, max_depth)
...
def search_plan(
    domain_file, problem_file, search, heuristic_class, max_depth:int=None, use_preferred_ops=False,
):
    """
    ...
    if use_preferred_ops and isinstance(heuristic, heuristics.hFFHeuristic):
        solution = _search(task, search, heuristic, max_depth=max_depth, use_preferred_ops=True)
    else:
        solution = _search(task, search, heuristic, max_depth=max_depth)
