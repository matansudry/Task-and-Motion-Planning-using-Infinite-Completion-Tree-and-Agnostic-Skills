...
class TableEnvEncoder(Encoder):
    ...
    def _get_observation_indices(
        ...
        #[MS] if there is issue is because of #@tensors.vmap(dims=0)
        
        observation_indices = np.array(policy_args["observation_indices"])

        if randomize:
            shuffle_range = policy_args["shuffle_range"]
            if len(observation_indices.shape) == 1:
                np.random.shuffle(observation_indices[shuffle_range[0] : shuffle_range[1]])
            elif len(observation_indices.shape) == 2:
                for index in range(observation_indices.shape[0]):
                    np.random.shuffle(observation_indices[index, shuffle_range[index, 0] : shuffle_range[index, 1]])
            else:
                raise
