...
class TableEnv(PybulletEnv):
    ...
    def __init__(
        ...
        #p.saveBullet('temp.txt') #p.restoreState('temp.txt')
    ...
    def reset(  # type: ignore
        ...
        *,
        ...
        cnt = 0
        for seed, options in tqdm.tqdm(self._seed_generator(seed)):
            cnt +=1
            if cnt >= 100000:
                raise
            ...
            if task_sampling_trials % max_samples_per_trial == 0:
                ...
                if task == None:
                    ...
                    while not_found:
                        ...
                        cnt +=1 
                        self._task = self.tasks.sample()
                        current_arg_objects = [item.name for item in self._task.action_skeleton[0].arg_objects]
                        current_initial_state = self._task.initial_state
                        if current_arg_objects == task['arg_objects'] and current_initial_state == task['initial_state']:
                            self.set_primitive(self.task.action_skeleton[0])
                            not_found = False
            ...
            for prop in self.task.initial_state:
                ok = prop.sample(self.robot, self.objects, self.task.initial_state)
                if not ok:
            ...
            if (
                ...
                dbprint(f"TableEnv.reset(seed={seed}): _is_any_object_below_table")
                continue
            if (
                self._is_any_object_touching_base()
            ):
                dbprint(f"TableEnv.reset(seed={seed}): _is_any_object_touching_base")
                continue
            if (
                self._is_object_inside_rack()
            ):
                dbprint(f"TableEnv.reset(seed={seed}): _is_object_inside_rack")
                continue
            if (
                self._is_any_object_falling_off_parent()
            ):
                dbprint(f"TableEnv.reset(seed={seed}): _is_any_object_falling_off_parent")
                continue
    ...
    def step(
        ...
        verbose:bool=False, get_video:bool=False) -> Tuple[np.ndarray, float, bool, bool, dict]:
    ...
    def _is_object_inside_rack(self) -> bool:
        for obj in self.objects.values():
            if obj.name == "rack":
                rack = obj
                break
        return any(
            utils._is_inside_rack(obj, rack)
            for obj in self.real_objects()
        )
