...
def is_below_table(obj: Object, sim: bool = True) -> bool:
    ...
    output = obj.pose(sim=sim).pos[2] < TABLE_CONSTRAINTS["table_z_max"]
    return output
...
def _is_inside_rack(obj:Object, rack:Object) -> bool:
    if obj.name == "rack":
        return False
    obj_pos = obj.pose(sim=True)
    obj_size = obj.size
    rack_pos = rack.pose(sim=True)
    rack_size = rack.size
    
    rack_height = rack_pos.pos[2]
    obj_limits = [
        obj_pos.pos[2] - obj_size[2]/2,
        obj_pos.pos[2] + obj_size[2]/2,
    ]
    if not(rack_height >= obj_limits[0] and rack_height <= obj_limits[1]):
        return False
    
    obj_box = [
        obj_pos.pos[0] - obj_size[0]/2,
        obj_pos.pos[1] - obj_size[1]/2,
        obj_pos.pos[0] + obj_size[0]/2,
        obj_pos.pos[1] + obj_size[1]/2,
    ]

    rack_box = [
        rack_pos.pos[0] - rack_size[0]/2,
        rack_pos.pos[1] - rack_size[1]/2,
        rack_pos.pos[0] + rack_size[0]/2,
        rack_pos.pos[1] + rack_size[1]/2,
    ]
    iou = bb_intersection_over_union(obj_box, rack_box)
    if iou == 0.0:
        return False
    else:
        return True