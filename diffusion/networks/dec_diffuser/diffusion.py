...
import diffusion.networks.dec_diffuser.utils as utils
from .helpers import (
...
def get_noise(tensor_like:torch.tensor, scale:float=1):
    noise = torch.randn_like(tensor_like)
    noise *= scale
    return noise
...
class GaussianDiffusion(nn.Module):
    ...
    def q_sample(self, x_start, t, noise=None):
        if noise is None:
            noise = get_noise(tensor_like=x_start, scale=self.noise_scale) #noise = torch.randn_like(x_start)
...
class GaussianInvDynDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, noise_scale, n_timesteps=1000,
        loss_type='l1', clip_denoised=False, predict_epsilon=True, hidden_dim=256,
        action_weight=1.0, loss_discount=1.0, loss_weights=None, returns_condition=False,
        condition_guidance_w=0.1, ar_inv=False, train_only_inv=False, diffusion_only=False,
    ...
    def get_loss_weights(self, discount):
        ...
        if self.use_forward_model:
            dim_weights = torch.ones(66, dtype=torch.float32)
        else:
...
class ActionGaussianDiffusion(nn.Module):
    ...
    def p_losses(self, action_start, state, t, returns=None):
        noise = get_noise(tensor_like=action_start, scale=self.noise_scale) #noise = torch.randn_like(action_start)
